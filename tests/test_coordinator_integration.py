...
def _stub_batch_client(calls):
    ...
    client.register_job_definition.side_effect = lambda **kw: (
        ...
        or calls.__setitem__("register_kwargs", kw)
        or {
    ...
    client.submit_job.side_effect = lambda **kw: (
        ...
        or calls.__setitem__("submit_kwargs", kw)
...
@moto.mock_aws
def test_immediate_submit_submits_once_and_does_not_poll(tmp_path, monkeypatch):
    # monkeypatch restores env + cwd on teardown; tmp_path is auto-cleaned, so
    # this test leaks no global process state into the rest of the suite.
    ...
    # ...and the submitted job is the coordinator rule specifically (its name is
    # baked into the registered job definition), not some other job body — so a
    # submit of the wrong rule would fail here rather than pass.
    assert COORDINATOR_RULE in str(calls["register_kwargs"])
