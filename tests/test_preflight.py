...
class TestQueueProblems:
    ...
    def test_regular_queue_without_compute_environment_reported(self):
        # A regular queue with no compute environments cannot run jobs (the
        # builder would default to EC2 and submit into a queue that never
        # dispatches), so preflight must flag it.
        ex = _with_queue(
            _executor(),
            {
                "state": "ENABLED",
        ...
        assert any("no compute environments" in p for p in problems)

    def test_sagemaker_service_queue_reported_as_unsupported(self):
        # A SAGEMAKER_TRAINING service queue legitimately has no compute
        # environments, but this executor submits compute-environment jobs and
        # cannot use it — flag it explicitly.
        ex = _with_queue(
            _executor(),
            {
                "state": "ENABLED",
        ...
        problems = ex._queue_problems()
        assert any("SAGEMAKER_TRAINING" in p for p in problems)
