...
data "aws_iam_policy_document" "executor_core" {
  # checkov:skip=CKV_AWS_356: The Batch Describe*/RegisterJobDefinition/TerminateJob
  # actions below do not support resource-level permissions (or target a job ARN
  # that is only known at runtime), so this statement must use "*". SubmitJob is
  # scoped to the generated queue and job definitions in the BatchSubmit statement.
  # Core: track, register and clean up Batch jobs
  statement {
    sid    = "BatchCore"
    effect = "Allow"
    actions = [
  ...
  # Core: submit jobs only to the queue and job definitions this stack manages.
  statement {
    sid     = "BatchSubmit"
    effect  = "Allow"
    actions = ["batch:SubmitJob"]
    resources = [
      aws_batch_job_queue.snakemake.arn,
