...
> **Relationship to `terraform/` at the repo root.**  The root `terraform/` module provisions a full standalone test environment — its own VPC, subnet, internet gateway, route table, security group, placement group, compute environment, and job queue, plus the ECS instance and Batch service roles — for exercising the plugin against real AWS Batch.  This `examples/terraform/` module is a *self-contained* onboarding stack for new users.  Both modules create ECS instance roles and Batch service roles with the same AWS managed policies; they do *not* share Terraform state.
...
| `container_image` | string | `ghcr.io/snakemake/snakemake-executor-plugin-aws-batch:main` | Container image every job runs; defaults to the `:main` tag (rebuilt on every push to `main`) because `:latest` does not exist until the first release. Prefer a pinned tag/digest for reproducibility once releases are published. Mirror into ECR if the compute environment has no egress to ghcr.io |
