...
variable "name_prefix" {
  ...
  validation {
    # AWS Batch names (queues, compute environments, job definitions) must start
    # with a letter or digit and cap out at 128 chars; this prefix is combined
    # with suffixes downstream, so keep it well under that at 38 chars.
    condition = (
      length(var.name_prefix) <= 38 &&
      can(regex("^[a-z0-9][a-z0-9-]*$", var.name_prefix))
    )
    error_message = "name_prefix must be at most 38 characters and start with a lowercase letter or digit, followed by lowercase letters, digits, and hyphens."
  }
}
...
variable "container_image" {
  ...
  type        = string
  default     = "ghcr.io/snakemake/snakemake-executor-plugin-aws-batch:main"
}
